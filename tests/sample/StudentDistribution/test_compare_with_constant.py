...
---> Class Test_two_side
    This class test if the all the outputs for a two-side test. The results are checked when the constant is distinct when compare to a mean, with values closer to the mean and far from the mean (but rejecting the H0). The tests checks language as well. Tests for real data and for paramters are included

---> Class Test_one_side_higher
    This class test if the all the outputs for a one-side test when is a upper test. The results are checked when the constant is distinct when compare to a mean, with values closer to the mean and far from the mean (but rejecting the H0). The tests checks language as well. Tests for real data and for paramters are included

---> Class Test_one_side_lower
    This class test if the all the outputs for a one-side test when is a lower test. The results are checked when the constant is distinct when compare to a mean, with values closer to the mean and far from the mean (but rejecting the H0). The tests checks language as well. Tests for real data and for paramters are included

---> Class Test_raises_error
    This class test when the method should raise ValueError. The output is also tested. Tests for real data and for paramters are included
...
class Test_raises_error(unittest.TestCase):
    ...
    def test_conflicting_params(self):
        with self.assertRaises(ValueError, msg="Does not raised error when x_exp and params is None"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(x_exp=None, params=None, value=self.value_lower, comparison='pvalue')
    ...
    def test_comparison_output(self):
        ...
        expected = "Parameters 'x_exp' and 'params' cannot be of type 'None' at the same time"
        result = False
        ...
        try:
            ...
            pass
        sys.stdout = sys.__stdout__
        expected = "Os parâmetros 'x_exp' e 'params' não podem ser do tipo 'None' ao mesmo tempo."
        result = False
        if expected in capturedOutput.getvalue():
            result = True
        self.assertTrue(result, msg="wrong output when Raising Error")
    ...
    def test_comparison(self):
        with self.assertRaises(ValueError, msg="Does not raised error when comparison is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, comparison='pvalue')

        with self.assertRaises(ValueError, msg="Does not raised error when comparison is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, comparison='None')

        with self.assertRaises(ValueError, msg="Does not raised error when comparison is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, comparison='critico')

        with self.assertRaises(ValueError, msg="Does not raised error when comparison is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, comparison='p-valor')

    def test_comparison_params(self):
        with self.assertRaises(ValueError, msg="Does not raised error when comparison is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(
                            params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                            value=self.value_lower, comparison='pvalue')

        with self.assertRaises(ValueError, msg="Does not raised error when comparison is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(
                            params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                            value=self.value_lower, comparison='None')

        with self.assertRaises(ValueError, msg="Does not raised error when comparison is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(
                            params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                            value=self.value_lower, comparison='critico')

        with self.assertRaises(ValueError, msg="Does not raised error when comparison is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(
                            params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                            value=self.value_lower, comparison='p-valor')
    ...
    def test_comparison_output_params(self):
        capturedOutput = io.StringIO()
        sys.stdout = capturedOutput
        try:
            student = StudentDistribution()
    ...
    def test_details(self):
        with self.assertRaises(ValueError, msg="Does not raised error when details is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, details='shot')

        with self.assertRaises(ValueError, msg="Does not raised error when details is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, details='ful')

        with self.assertRaises(ValueError, msg="Does not raised error when details is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, details='binario')
    ...
    def test_details_params(self):
        with self.assertRaises(ValueError, msg="Does not raised error when details is wrong"):
            ...
            result, conclusion = student.compare_with_constant(
                            params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                            value=self.value_lower, details='shot')

        with self.assertRaises(ValueError, msg="Does not raised error when details is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(
                            params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                            value=self.value_lower, details='ful')

        with self.assertRaises(ValueError, msg="Does not raised error when details is wrong"):
            student = StudentDistribution()
            result, conclusion = student.compare_with_constant(
                            params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                            value=self.value_lower, details='binario')
    ...
    def test_details_output_params(self):
        capturedOutput = io.StringIO()
        sys.stdout = capturedOutput
        try:
            student = StudentDistribution()
...
class Test_one_side_lower(unittest.TestCase):
    ...
    def test_samples_distinct_default(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, which="one-side")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_default_params(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_lower, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_params(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
        value=self.value_lower, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_p_value_params(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_lower, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_p_value_params(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_lower, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_default(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_default_params(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full_params(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full_p_value_params(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_p_value_params(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_binary(self):
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, which="one-side", details="binary")
        ...
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_lower, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")
    ...
    def test_binary_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_lower, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_lower, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")
...
class Test_one_side_higher(unittest.TestCase):
    ...
    def test_samples_distinct_default(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_default_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_p_value_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_p_value(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_default(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_default_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, which="one-side")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, which="one-side", details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full_p_value_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, which="one-side", details="full", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_p_value_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, which="one-side", comparison='p-value')
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_binary(self):
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")
    ...
    def test_binary_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, which="one-side", details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, details="binary", which="one-side", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is rejected")
...
class Test_two_side(unittest.TestCase):
    ...
    def test_samples_distinct_default(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value)
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower)
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher)
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher)
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_full_p_value_params(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_distinct_p_value(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equal_default(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher)
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower)
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, details="full")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_full_p_value(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, details="full", comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_p_value(self):
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        student = StudentDistribution(language='pt-br')
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_samples_equals_p_value(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, comparison="p-value")
        self.assertIsInstance(result, tuple, msg="result not a tuple")
        self.assertIsInstance(result[0], float, msg="statistic not a float")
        self.assertIsInstance(result[1], list, msg="critical not list")
        self.assertIsInstance(result[1][1], float, msg="critical lower not float")
    ...
    def test_binary(self):
        ...
        student = StudentDistribution()
        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_higher, details="binary", comparison="p-value")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_equal_lower, details="binary", comparison="p-value")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value, details="binary", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_lower, details="binary", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_higher, details="binary", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(self.x, value=self.value_very_higher, details="binary", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")
    ...
    def test_binary(self):
        ...
        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_higher, details="binary", comparison="p-value")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_equal_lower, details="binary", comparison="p-value")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 0, msg="conclusion not 0 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value, details="binary", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_lower, details="binary", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_higher, details="binary", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, details="binary")
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")

        result, conclusion = student.compare_with_constant(
                        params=[self.x.mean(), self.x.std(ddof=1), self.x.size],
                        value=self.value_very_higher, details="binary", comparison='p-value')
        self.assertIsInstance(conclusion, int, msg="conclusion not a int")
        self.assertEqual(conclusion, 1, msg="conclusion not 1 when H0 is accepted")
